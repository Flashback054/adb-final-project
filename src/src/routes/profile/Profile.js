...
const Profile = ({ currentUser, handleLogout, updateUser }) => {
  ...
  useEffect(() => {
    document.title = "Profile || Sushi Time";
  });
      ...
      {loading ? (
        ...
        <React.Fragment>
          <article className="profile__info">
            {[
              { label: "Tên Đăng Nhập", value: currentUser.TenDangNhap },
              { label: "Email", value: currentUser.Email },
              { label: "Họ Tên", value: currentUser.HoTen },
              { label: "Giới Tính", value: currentUser.GioiTinh },
              { label: "CCCD", value: currentUser.CCCD },
              {
                label: "Address",
                value:
                  currentUser.address !== null
                    ? `${currentUser.Duong}, ${currentUser.Phuong}, ${currentUser.Quan}, ${currentUser.ThanhPho}`
                    : "N/A",
              },
              { label: "Số Điện Thoại", value: currentUser.SDT || "N/A" },
            ].map((item, index) => (
              <section key={index} className="profile__info__section">
                <h3>{item.label}</h3>
                <p>{item.value}</p>
              </section>
            ))}
